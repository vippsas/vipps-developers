...
[`PUT:/ecomm/v2/payments/{orderId}/cancel`](/api/ecom#tag/Vipps-eCom-API/operation/cancelPaymentRequestUsingPUT)
request with `shouldReleaseRemainingFunds: true` in the body.
The payment must be `RESERVED` for this to take effect.
See:
[Cancelling a partially captured order](/docs/APIs/ecom-api/vipps-ecom-api#cancelling-a-partially-captured-order).

## The Vipps QR API now works in the test environment

[The Vipps QR API](/docs/APIs/qr-api) now works in both in
the production and
...
The response of the
[`GET:/v1/merchants/{merchantSerialNumber}`](/api/psp-signup#tag/Merchant/operation/getMerchants)
endpoint in the
[Vipps PSP API](/docs/APIs/psp-api)
...
## Recurring charge changes
 ...
 See:
 [Create charge](/docs/APIs/recurring-api/vipps-recurring-api#create-charge).
